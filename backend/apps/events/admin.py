...
class EventAdmin(admin.ModelAdmin):
    list_display = ('title', 'event_type', 'status', 'start_datetime',
                    'location', 'attendee_count', 'is_full_display')

    list_filter = ('event_type', 'status', 'requires_registration',
    ...
    def is_full_display(self, obj):
        if obj.is_full:
            ...
            return format_html(
                '<span style="color: orange;">Casi lleno</span>')
...
class EventAttendanceAdmin(admin.ModelAdmin):
    ...
    fieldsets = (('Información del Evento', {
        'fields': ('event', )
