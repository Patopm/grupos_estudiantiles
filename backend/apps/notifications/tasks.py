...
def send_email_notification(self, notification_id):
    ...
    try:
        ...
        success = send_mail(
            ...
            message=notification.text_content or strip_tags(notification.html_content),
...
def cleanup_old_notifications():
    ...
    sent_deleted = EmailNotification.objects.filter(
        status="sent", sent_at__lt=sent_cutoff
    ).delete()[0]
    ...
    failed_deleted = EmailNotification.objects.filter(
        status="failed", failed_at__lt=failed_cutoff
    ).delete()[0]
    ...
    for user in daily_users:
        try:
            ...
            notifications = EmailNotification.objects.filter(
                recipient=user, created_at__gte=yesterday, status="pending"
            ).order_by("-created_at")
            ...
            if notifications.exists():
                ...
                success = NotificationService.send_daily_digest(user, notifications)
...
def send_weekly_digest():
    ...
    for user in weekly_users:
        try:
            ...
            notifications = EmailNotification.objects.filter(
                recipient=user, created_at__gte=last_week, status="pending"
            ).order_by("-created_at")
            ...
            if notifications.exists():
                ...
                success = NotificationService.send_weekly_digest(user, notifications)
