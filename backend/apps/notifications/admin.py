...
class EmailNotificationAdmin(admin.ModelAdmin):
    ...
    list_filter = ["status", "priority", "created_at", "scheduled_at", "sent_at"]
    search_fields = [
        "recipient__email",
    ...
    fieldsets = (
        (
            ...
            {"fields": ("recipient", "template", "subject", "from_email", "reply_to")},
        ),
        ("Configuración", {"fields": ("priority", "scheduled_at", "status")}),
        (
            "Contenido del Email",
            {"fields": ("html_preview", "text_preview"), "classes": ("collapse",)},
        ),
        ("Datos de Contexto", {"fields": ("context_data",), "classes": ("collapse",)}),
        (
            "Estado y Errores",
            {
...
class EventReminderAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Estado", {"fields": ("sent", "sent_at")}),
        ("Fechas", {"fields": ("created_at",), "classes": ("collapse",)}),
    )

    def get_queryset(self, request):
        return super().get_queryset(request).select_related("event", "recipient")
