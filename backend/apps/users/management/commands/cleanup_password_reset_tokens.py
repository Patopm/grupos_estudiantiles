...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING(f'Would delete {count} expired password reset tokens')
            )
            if count > 0:
                self.stdout.write('Expired tokens:')
                for token in expired_tokens[:10]:  # Show first 10
                    self.stdout.write(f'  - {token.user.email} (expired: {token.expires_at})')
                if count > 10:
                    self.stdout.write(f'  ... and {count - 10} more')
        else:
            deleted_count = PasswordResetToken.cleanup_expired_tokens()
            self.stdout.write(
                self.style.SUCCESS(f'Successfully deleted {deleted_count} expired password reset tokens')
            )