        ...
        for policy_data in default_policies:
            ...
            try:
                ...
                if force:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"Updated MFA policy for {role}: {description}"
                        )
                    )
                else:
                    self.stdout.write(
                        self.style.WARNING(
                            f"Policy for {role} already exists. Use --force to update."
                        )
                    )
            except MFAEnforcementPolicy.DoesNotExist:
                # Create new policy
                policy_data["enforcement_date"] = (
                    timezone.now() if policy_data["mfa_required"] else None
                )
