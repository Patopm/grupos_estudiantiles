...
def lock_ip(ip, duration=None):
    ...
    """
    if duration is None:
        duration = getattr(settings, 'SECURITY_SETTINGS', {}).get('IP_LOCKOUT_DURATION', 3600)
...
def lock_user(user_identifier, duration=None):
    ...
    """
    if duration is None:
        duration = getattr(settings, 'SECURITY_SETTINGS', {}).get('USER_LOCKOUT_DURATION', 1800)
...
def log_security_event(request, event_type, extra_data=None):
    ...
    event_key = f"security:events:{int(time.time() * 1000)}"  # Use milliseconds for uniqueness
...
def check_suspicious_patterns(request):
    ...
    if len(requests) > 100:  # More than 100 requests per minute
        log_security_event(request, 'rapid_requests', {
    ...
    if any(agent in user_agent for agent in suspicious_agents):
        log_security_event(request, 'suspicious_user_agent', {
    ...
    if not request.META.get('HTTP_ACCEPT'):
        log_security_event(request, 'missing_headers', {
            'missing_header': 'Accept'
        })
        return True
...
def validate_request_integrity(request):
    ...
    for header in dangerous_headers:
        if header in request.META:
            log_security_event(request, 'dangerous_header', {
                'header': header,
...
class SecurityMonitor:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type:
            log_security_event(self.request, f'{self.operation_type}_error', {
                'error_type': exc_type.__name__,
                'duration': duration
            })
        else:
            log_security_event(self.request, f'{self.operation_type}_success', {
                'duration': duration
            })
