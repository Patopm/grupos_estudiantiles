...
from .security_utils import (get_client_ip, is_ip_locked, is_user_locked,
                             lock_ip, lock_user, increment_failure_count,
                             get_failure_count, log_security_event,
...
class SecurityMiddleware(MiddlewareMixin):
            ...
            try:
                ...
                if username:
                    ...
                    if user_failures >= max_user_attempts:
                        lock_user(username)
                        log_security_event(request, 'user_locked', {
                            'username': username,
    ...
    def _handle_permission_violation(self, request, response):
        ...
        AuditLog.log_permission_violation(request=request,
                                          user=user,
                                          resource=resource,
                                          action=action,
                                          extra_data={
    ...
    def process_request(self, request):
        ...
        if self._is_sensitive_endpoint(request):
            AuditLog.log_event(
                ...
                if self._is_admin_endpoint(request) else 'unauthorized_access',
                request=request,
