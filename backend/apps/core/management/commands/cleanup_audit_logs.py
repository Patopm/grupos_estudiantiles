...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if keep_unresolved:
            queryset = queryset.exclude(resolved=False,
        ...
        if total_count == 0:
            self.stdout.write(
                self.style.SUCCESS('No audit logs found for cleanup.'))
            return

        # Show summary
        self.stdout.write(
            f'Found {total_count} audit logs older than {days} days')
        ...
        if not options.get(
                'verbosity',
                1) == 0:  # Skip confirmation in non-interactive mode
            confirm = input(
