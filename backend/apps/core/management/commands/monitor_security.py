...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not any(
            [options["clear_locks"], options["show_events"], options["show_limits"]]
        ):
    ...
    def clear_security_locks(self):
        ...
        try:
            ...
            if isinstance(cache, RedisCache):
                ...
                self.stdout.write(
                    self.style.SUCCESS("Security locks cleared successfully")
                )
            else:
                self.stdout.write(
                    self.style.WARNING(
                        "Cache backend not supported for bulk operations"
                    )
                )
            ...
            if isinstance(cache, RedisCache):
                ...
                for event in events[:20]:
                    timestamp = time.strftime(
                        "%Y-%m-%d %H:%M:%S", time.localtime(event.get("timestamp", 0))
                    )
                    self.stdout.write(
                ...
                self.stdout.write(
                    self.style.WARNING("Cache backend not supported for event listing")
                )
        except Exception as e:
        ...
        try:
            ...
            if isinstance(cache, RedisCache):
                ...
                if specific_ip:
                    ...
                    if throttle_keys:
                        self.stdout.write(
                            f"Found {len(throttle_keys)} active rate limits"
                        )
                ...
                self.stdout.write(
                    self.style.WARNING(
                        "Cache backend not supported for rate limit listing"
                    )
                )
        except Exception as e:
        ...
        try:
            ...
            if isinstance(cache, RedisCache):
                ...
                for scope, config in rate_limits.items():
                    self.stdout.write(
                        f"  {scope}: {config['requests']} requests per {config['window']}s"
                    )

            else:
                self.stdout.write(
                    self.style.WARNING("Cache backend not supported for summary")
                )
        except Exception as e:
