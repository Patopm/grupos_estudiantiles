...
class Command(BaseCommand):
    ...
    def _generate_text_report(self, queryset, hours):
        ...
        self.stdout.write(f'Unresolved Critical: {summary["unresolved_critical"]}')
        ...
        critical_events = queryset.filter(severity__in=["critical", "high"]).order_by(
            "-timestamp"
        )[:10]
        ...
        if critical_events:
            ...
            for event in critical_events:
                ...
                self.stdout.write(
                    ...
                    f"{event.ip_address} - "
                    f"{status}",
                    self._get_severity_color(event.severity),
                )
                if event.message:
        ...
        if auth_failures > 0:
            ...
            failing_ips = (
                queryset.filter(event_type__in=["login_failed", "token_refresh_failed"])
                .values("ip_address")
        ...
        permission_violations = queryset.filter(event_type="permission_denied").count()
