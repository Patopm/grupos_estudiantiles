...
class AuditLog(models.Model):
    ...
    username = models.CharField(
        ...
        blank=True,
        help_text=
        "Username at the time of the event (preserved even if user is deleted)"
    )
    ...
    message = models.TextField(
        help_text="Detailed message describing the event")
    ...
    def log_event(cls,
        ...
        """
        from apps.core.security_utils import (get_client_fingerprint,
                                              get_client_ip)

        # Extract request information
        ip_address = get_client_ip(request) if request else None
        user_agent = request.META.get('HTTP_USER_AGENT', '') if request else ''
        session_key = request.session.session_key if request and hasattr(
        ...
        if not user and request and hasattr(
    ...
    @classmethod
    def log_authentication_event(cls,
                                 event_type,
                                 request,
                                 user=None,
                                 success=True,
        ...
        message_map = {
            'login_success':
            f"User {user.username if user else 'unknown'} logged in successfully",
            'login_failed':
            f"Failed login attempt for user {extra_data.get('username', 'unknown') if extra_data else 'unknown'}",
            'logout':
            f"User {user.username if user else 'unknown'} logged out",
            'token_refresh':
            f"Token refreshed for user {user.username if user else 'unknown'}",
            'token_refresh_failed': "Token refresh failed",
            'register_success':
            f"New user {user.username if user else 'unknown'} registered successfully",
        ...
        return cls.log_event(event_type=event_type,
                             request=request,
                             user=user,
                             message=message,
    ...
    def log_security_event(cls,
        ...
        return cls.log_event(event_type=event_type,
                             ...
                             user=user,
                             message=message
                             or f"Security event: {event_type}",
                             extra_data=extra_data,
                             severity=severity)

    @classmethod
    def log_permission_violation(cls,
                                 request,
                                 user=None,
                                 resource="",
                                 action="",
        ...
        return cls.log_security_event(event_type='permission_denied',
                                      request=request,
                                      user=user,
                                      message=message,
    ...
    def get_security_summary(cls, hours=24):
        ...
        summary = {
            ...
            'by_type': {},
            'unique_ips':
            events.values('ip_address').distinct().count(),
            'unique_users':
            events.filter(
                user__isnull=False).values('user').distinct().count(),
            'unresolved_critical':
            events.filter(severity='critical', resolved=False).count(),
            'unresolved_high':
            events.filter(severity='high', resolved=False).count(),
        }

        # Count by severity
        for severity, _ in cls.SEVERITY_LEVELS:
            summary['by_severity'][severity] = events.filter(
                severity=severity).count()

        # Count by event type (top 10)
        from django.db.models import Count
        event_counts = events.values('event_type').annotate(
            count=Count('event_type')).order_by('-count')[:10]
